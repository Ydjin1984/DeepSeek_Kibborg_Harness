...
export class LocalPtySession implements TerminalBackendSession {
  ...
  private async beginSend(operation: LocalSendOperation, request: TerminalSendRequest): Promise<void> {
    ...
    try {
      ...
      const terminator = request.submit ? '\r' : ''
      if (request.text.length + terminator.length > 0 && !operation.cancelRequested) {
        this.resetReadinessEvidence()
        const write = request.text.length === 0
          ? this.terminal.write(terminator)
          : this.terminal.write(request.text).then(async () => {
            if (terminator.length === 0) return
            // DIAG: submit as its own write, after the echoed line settles.
            await new Promise((resolve) => { setTimeout(resolve, 80) })
            await this.terminal.write(terminator)
          })
