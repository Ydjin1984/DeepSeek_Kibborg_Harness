...
- **A provider that authenticates through OAuth alone is not offered in the dormant directory** — `openai-codex` is the only such catalog route. A route that also takes an API key (xAI) appears normally: the Models page offers subscription device-code sign-in beside the key field. OAuth tokens are stored as JSON under `<ROUTE>_OAUTH` and refreshed per request; login never reads another product's auth file. Codex remains withheld because this adapter still has no refresh loop for an expiring Codex token.
