...
- **Read-side status projection** — registers the `compaction` session projection unit: the `auto` flag, the resolved `thresholdRatio` for the current route (exact `modelPolicies` override or the top-level default), and the in-flight `active` lock. The unit folds only the `compaction/start…end` bracket, `request/context` route records, and the `session/end-seed` stale-orphan reset, so a browser surface can gate a manual-compaction control on the capability, tint at the auto threshold, and disable while a compaction is in flight. Registration is an optional child of the engine (compositions without `ctx.sessionProjections` keep the standalone read shape).
