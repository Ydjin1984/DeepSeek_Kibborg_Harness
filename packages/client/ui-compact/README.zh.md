...
间接，通过按钮所触发的宿主 /compact 命令：其处理程序执行一次摘要压缩——模型调用与表层替换均为压缩后端自己的行为，记录为 `compaction/start…end` 括号及检查点的 `user/message`。

#### KV Cache 影响

按钮本身无影响。它触发的压缩复用对话的系统提示、工具与前置消息进行摘要调用（后端行为），从而保留已摘要前缀的提供方热 KV cache。
