...
import type { CallId, ChatStoreState, ExecutionExpandMode, SelectionTarget } from './contract/views.ts'
...
type ChatActions = {
  ...
  setExecutionExpand: (draft: ChatStoreState, mode: ExecutionExpandMode) => void
}
...
export function createChatStore(): EngineStoreHandle<ChatStoreState, ChatActions> {
  return defineStore({
    ...
    init: (): ChatStoreState => ({ selection: null, draft: '', view: null, inspect: null, executionExpand: 'default' }),
    ...
    actions: {
      ...
      setExecutionExpand: (d, mode: ExecutionExpandMode) => { d.executionExpand = mode },
    },
  })
}
