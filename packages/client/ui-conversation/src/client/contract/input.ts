...
export interface SessionInput extends InputTarget {
  ...
  /** Remove one browser-owned draft file; busy admission phases refuse. */
  removeFile(id: DraftAttachmentId): void
  /** Drop ids whose browser-owned file objects no longer exist. */
...
export interface InputActions {
  ...
  /**
   * Mint browser-owned draft files (any format) and append their ids; busy
   * admission phases refuse. The paperclip flow's public route — minting
   * lives inside because the bytes stay in the conversation controller.
   * @param files - browser files to attach.
   * @returns null when every file was attached, or a rejection reason string.
   */
  addFiles(files: readonly File[]): string | null
...
export interface InputState {
  ...
  /** Ordered runtime-only file ids (any format); bytes stay in ConversationController. */
