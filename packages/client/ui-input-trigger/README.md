...
`detectTrigger` treats `/` and `@` inside a user-authored triple-backtick code block as literal text while the caret is there; detection resumes after its closing fence. An unfinished block keeps triggers inactive through the draft's end.
