...
每次 history RPC 最多返回 400 条原始事件。`Session.open()` 请求最近 500 条 append 来源的用户与 assistant 消息，连续获取分页，直到取得 500 条或到达日志开头；每次 `loadOlder()` 最多再增加 100 条消息。分片密集的输出达到事件上限时，一页可包含少于请求数量的消息。已加载窗口保持连续；空页或失败页会终止当前批次，不进入重试循环。实时保留至少覆盖已打开的消息数量，包括用户主动加载的历史；它只在消息完成后裁剪，而为了保留这些消息，原始事件数量可以超过目标值。
