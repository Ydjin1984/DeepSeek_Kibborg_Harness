...
| `preparedSessionCacheMaxEvents` | positive integer (default `500_000`) | Maximum stored-event count those retained Sessions may hold in total. Each retained Session is one whole decoded log and packed chunk rows expand roughly thirtyfold when decoded, so the entry count alone does not bound the heap: one log heavier than this budget is never retained, and the oldest ready entry is evicted until the total fits. A budget below the size of a working session disables the reuse it exists for: every read of that session re-decodes its log, which costs seconds per page while a client pages back through the transcript. |
