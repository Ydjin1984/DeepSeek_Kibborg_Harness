...
| `preparedSessionCacheMaxEvents` | 正整数（默认 `500_000`） | 这些保留会话总共可持有的已存储事件数上限。每个保留会话都是一份完整解码的日志，而打包分片行解码后会膨胀约三十倍，因此只限制条目数并不能约束堆：比该预算更重的日志根本不会被保留，最旧的 ready 条目会被逐出直到总量落在预算内。预算低于一个工作会话的事件数，就会让它原本提供的复用失效：该会话的每一次读取都要重新解码日志，在客户端逐页回看 transcript 时每页要花上数秒。 |
