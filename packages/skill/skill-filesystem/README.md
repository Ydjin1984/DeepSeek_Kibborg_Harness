...
Skills can be single-level directory bundles (`<name>/SKILL.md`) or flat Markdown files (`<name>.md`). Nested `**/SKILL.md` discovery is deliberately excluded. Frontmatter is parsed as an open YAML object with the `yaml` package; this provider interprets required `name` and `description`, plus optional `whenToUse`, `metadata`, `description.zh`, `description.ru`, `disable-model-invocation`, and `user-invocable`. Names must be kebab-case. The optional `description.zh` / `description.ru` keys are per-locale routing descriptions: locale-aware discovery consumers (the web skill menu) prefer the active locale's entry and fall back to the base `description`, which remains the model-facing default and is never replaced by a localized one.
