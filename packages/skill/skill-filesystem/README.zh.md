...
skill 可以是单层目录 bundle（`<name>/SKILL.md`），也可以是平铺 Markdown 文件（`<name>.md`）。刻意不支持发现嵌套的 `**/SKILL.md`。Frontmatter 使用 `yaml` 包解析为开放的 YAML 对象；该提供方解析必填的 `name` 和 `description`，以及可选的 `whenToUse`、`metadata`、`description.zh`、`description.ru`、`disable-model-invocation` 和 `user-invocable`。名称必须使用 kebab-case。可选的 `description.zh` / `description.ru` 键是按语言的路由描述：感知语言的发现消费端（web 技能菜单）优先使用当前语言对应的条目，并回退到基础 `description`——后者仍是面向模型的默认值，绝不被本地化版本替换。
