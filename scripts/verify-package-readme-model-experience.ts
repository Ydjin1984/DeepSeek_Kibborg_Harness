...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-compact': { kind: 'indirect', reason: 'The compact button dispatches the host /compact command; the compaction backend owns the summarization call, session events, and surface replacement.' },
  'packages/client/ui-files': { kind: 'indirect', reason: 'The file panel folds dropped-file citations into the host draft-message path; the host owns the durable user message and its model-visible rendering.' },
  ...
  'packages/client/ui-settings-skills': { kind: 'none', reason: 'Browser-side settings surface; benchmark tasks execute in the host skill-manager service, which documents its own model effects.' },
  ...
  'packages/host/apiproxy': { kind: 'indirect', reason: 'The file-attachment admission path folds attached-file descriptions into the session user message; the session owns the assembled provider request.' },
