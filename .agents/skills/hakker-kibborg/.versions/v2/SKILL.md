...
description.ru: "Автономный red-team движок: ищет реальные цепочки атак до максимального подтверждённого компрометирования — не просто CVE. Активируется на /hakker-kibborg или запросах red-team assessment, пентеста, исследования уязвимостей, discovery атакующей поверхности и оценки maximum compromise."
description.zh: "自主红队引擎：不只找 CVE，而是追踪到最大已确认影响范围的真实攻击链，覆盖侦察、网络、Web、API、认证/授权、数据库、文件系统、凭据、提权、后渗透和横向移动。用户调用 /hakker-kibborg 或请求红队评估、渗透测试、漏洞研究、攻击面发现或最大影响评估时使用。"
whenToUse:
  - "User invokes /hakker-kibborg"
  - "User asks for a red-team assessment, penetration test, or maximum-compromise evaluation"
  - "User asks for vulnerability research or attack-surface discovery"
metadata:
  category: security
  tags: [red-team, pentest, offensive, exploit, c2]
---
