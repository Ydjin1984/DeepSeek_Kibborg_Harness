...
- **SessionManager** (manager.ts): instance cluster + frame entry + the session list. sessionId-bearing frames go only to existing instances (a mux broadcast must not instantiate every session); approval/question `requested` frames are the exception — they never land in history, so they buffer in `pendingBuffers` and replay on instantiation. Running subagent children are the other deliberate exception: their windows auto-open (catalog refresh completion, `host/session-status` running, or the first `session/event` frame) so their transcripts stay live off-screen ([live background subagents](../bug-fix/2026-08-30-web-live-background-subagents.md)).
