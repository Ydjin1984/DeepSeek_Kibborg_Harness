...
- **SessionManager**（manager.ts）：实例簇 + 帧总入口 + 会话列表。带 sessionId 的帧只投已存在实例（mux 广播不得把每个会话都实例化）；例外是审批/问答 `requested` 帧——它们不落 history、open 无法回补，故缓冲进 `pendingBuffers`，实例化时回放。运行中的 subagent 子级是另一个有意为之的例外：其窗口自动打开（目录刷新完成、`host/session-status` 为 running、或首个 `session/event` 帧），使转录在后台保持实时（[后台 subagent 实时可见](../bug-fix/2026-08-30-web-live-background-subagents.md)）。
