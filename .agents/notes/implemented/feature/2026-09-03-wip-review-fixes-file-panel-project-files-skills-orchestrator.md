...
- **Workspace file panel (`ui-files`)** — tree rows render base names for POSIX and Windows paths (`baseNameOf`); rows carry VSCode-style glyphs (an amber folder with an open state, monogram badges for common source/config formats, a neutral page otherwise) with colours in the CSS module; hidden entries are visually muted; `listChildren` failures surface as an inline row with a retry instead of an empty folder; `dragover` inspects only `dataTransfer.types` (Chromium returns an empty `getData` before `drop`); a drop stamps `COMPOSER_FILE_SOURCE_PATH` on the draft `File` so `session.prompt` cites the original project path instead of copying under `.dsh/attachments/` (an out-of-project `path` is ignored and materialized as a copy); the Markdown dialog cannot resurrect itself when a save settles after close.
...
- `ui-files`: tests incl. Windows/POSIX base names, icon glyph variants, failed-listing retry, hidden rows, Chromium-safe dragover/drop (stamped project path), save-after-close.
...
- Host tsdown: `tsdown.config.ts` workspace exclude restates tsdown's defaults plus `**/graphify-out/**`, so a `graphify update packages` cache under `packages/graphify-out/` cannot be treated as a workspace package.
- Client bundle: `ui-files` stamps `COMPOSER_FILE_SOURCE_PATH` (`dshSourcePath`) locally rather than value-importing `ui-conversation/client`, which the purity gate rejects.
